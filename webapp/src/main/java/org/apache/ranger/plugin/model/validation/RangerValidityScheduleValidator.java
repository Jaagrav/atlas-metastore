...
public class RangerValidityScheduleValidator {
    ...
    public RangerValiditySchedule validate(List<ValidationFailureDetails> validationFailures) {
        ...
        if (StringUtils.isEmpty(validitySchedule.getStartTime()) && StringUtils.isEmpty(validitySchedule.getEndTime()) && CollectionUtils.isEmpty(validitySchedule.getRecurrences())) {
            ...
            if (startTime != null && endTime != null) {
                validityPeriodEstimator = new RecurrenceSchedule();
    ...
    private boolean validateTimeRangeSpec(List<ValidationFailureDetails> validationFailures) {
        ...
        for (RangerValidityRecurrence recurrence : validitySchedule.getRecurrences()) {
            ...
            if (ret) {
                ret = validateFieldSpec(recurrence, RecurrenceSchedule.ScheduleFieldSpec.minute, validationFailures) && ret;
                ret = validateFieldSpec(recurrence, RecurrenceSchedule.ScheduleFieldSpec.hour, validationFailures) && ret;
                ret = validateFieldSpec(recurrence, RecurrenceSchedule.ScheduleFieldSpec.dayOfMonth, validationFailures) && ret;
                ret = validateFieldSpec(recurrence, RecurrenceSchedule.ScheduleFieldSpec.dayOfWeek, validationFailures) && ret;
                ret = validateFieldSpec(recurrence, RecurrenceSchedule.ScheduleFieldSpec.month, validationFailures) && ret;
                ret = validateFieldSpec(recurrence, RecurrenceSchedule.ScheduleFieldSpec.year, validationFailures) && ret;
                ret = ret && validateIntervalDuration(recurrence, validationFailures);

                if (ret) {
                    RecurrenceSchedule schedule = new RecurrenceSchedule(getNormalizedValue(recurrence, RecurrenceSchedule.ScheduleFieldSpec.minute), getNormalizedValue(recurrence, RecurrenceSchedule.ScheduleFieldSpec.hour),
                            getNormalizedValue(recurrence, RecurrenceSchedule.ScheduleFieldSpec.dayOfMonth), getNormalizedValue(recurrence, RecurrenceSchedule.ScheduleFieldSpec.dayOfWeek),
                            getNormalizedValue(recurrence, RecurrenceSchedule.ScheduleFieldSpec.month), getNormalizedValue(recurrence, RecurrenceSchedule.ScheduleFieldSpec.year));
    ...
    private boolean validateFieldSpec(RangerValidityRecurrence recurrence, RecurrenceSchedule.ScheduleFieldSpec field, List<ValidationFailureDetails> validationFailures) {
        boolean ret = true;
        ...
        if (StringUtils.isBlank(fieldValue)) {
            ...
            if (StringUtils.equals(field.name(), RecurrenceSchedule.ScheduleFieldSpec.dayOfWeek.name())
                    || StringUtils.equals(field.name(), RecurrenceSchedule.ScheduleFieldSpec.dayOfMonth.name())) {
                if (LOG.isDebugEnabled()) {
        ...
        if (!ret) {
            ...
            int minimum = field == RecurrenceSchedule.ScheduleFieldSpec.month ? field.minimum + 1 : field.minimum;
            int maximum = field == RecurrenceSchedule.ScheduleFieldSpec.month ? field.maximum + 1 : field.maximum;
    ...
    private boolean validateIntervalDuration(RangerValidityRecurrence recurrence, List<ValidationFailureDetails> validationFailures) {
        ...
        if (!validationFailures.isEmpty() || validityPeriodEstimator == null) {
            ...
            String minutes = validityPeriodEstimator.getFieldValue(RecurrenceSchedule.ScheduleFieldSpec.minute);
            if (!StringUtils.equals(minutes, RecurrenceSchedule.WILDCARD)) {
                minSchedulingInterval = StringUtils.isBlank(minutes) ? RecurrenceSchedule.ScheduleFieldSpec.minute.maximum + 1 : Integer.valueOf(minutes);

                if (minSchedulingInterval == RecurrenceSchedule.ScheduleFieldSpec.minute.maximum + 1) {
                    String hours = validityPeriodEstimator.getFieldValue(RecurrenceSchedule.ScheduleFieldSpec.hour);
                    if (!StringUtils.equals(hours, RecurrenceSchedule.WILDCARD)) {
                        int hour = StringUtils.isBlank(hours) ? RecurrenceSchedule.ScheduleFieldSpec.hour.maximum + 1 :Integer.valueOf(hours);
                        minSchedulingInterval = hour * (RecurrenceSchedule.ScheduleFieldSpec.minute.maximum+1);

                        if (hour == RecurrenceSchedule.ScheduleFieldSpec.hour.maximum + 1) {
                            String dayOfMonths = validityPeriodEstimator.getFieldValue(RecurrenceSchedule.ScheduleFieldSpec.dayOfMonth);
                            String dayOfWeeks = validityPeriodEstimator.getFieldValue(RecurrenceSchedule.ScheduleFieldSpec.dayOfWeek);

                            int dayOfMonth = 1, dayOfWeek = 1;
                            if (!StringUtils.equals(dayOfMonths, RecurrenceSchedule.WILDCARD)) {
                                dayOfMonth = StringUtils.isBlank(dayOfMonths) ? RecurrenceSchedule.ScheduleFieldSpec.dayOfMonth.maximum + 1 : Integer.valueOf(dayOfMonths);
                            }
                            if (!StringUtils.equals(dayOfWeeks, RecurrenceSchedule.WILDCARD)) {
                                dayOfWeek = StringUtils.isBlank(dayOfWeeks) ? RecurrenceSchedule.ScheduleFieldSpec.dayOfWeek.maximum + 1 : Integer.valueOf(dayOfWeeks);
                            }
                            if (!StringUtils.equals(dayOfMonths, RecurrenceSchedule.WILDCARD) || !StringUtils.equals(dayOfWeeks, RecurrenceSchedule.WILDCARD)) {
                                int minDays = dayOfMonth > dayOfWeek ? dayOfWeek : dayOfMonth;
                                minSchedulingInterval = minDays*(RecurrenceSchedule.ScheduleFieldSpec.hour.maximum+1)*(RecurrenceSchedule.ScheduleFieldSpec.minute.maximum+1);

                                if (dayOfMonth == (RecurrenceSchedule.ScheduleFieldSpec.dayOfMonth.maximum+1) && dayOfWeek == (RecurrenceSchedule.ScheduleFieldSpec.dayOfWeek.maximum+1)) {
                                    String months = validityPeriodEstimator.getFieldValue(RecurrenceSchedule.ScheduleFieldSpec.month);
                                    if (!StringUtils.equals(months, RecurrenceSchedule.WILDCARD)) {
                                        int month = StringUtils.isBlank(months) ? RecurrenceSchedule.ScheduleFieldSpec.month.maximum + 1 :Integer.valueOf(months);
                                        minSchedulingInterval = month * 28 * (RecurrenceSchedule.ScheduleFieldSpec.hour.maximum + 1) * (RecurrenceSchedule.ScheduleFieldSpec.minute.maximum + 1);

                                        if (month == RecurrenceSchedule.ScheduleFieldSpec.month.maximum + 1) {
                                            // Maximum interval is 1 year
                                            minSchedulingInterval = 365 * (RecurrenceSchedule.ScheduleFieldSpec.hour.maximum + 1) * (RecurrenceSchedule.ScheduleFieldSpec.minute.maximum + 1);
                                        }
                                    }
                                }
                            }
    ...
    private boolean validateRanges(RangerValidityRecurrence recurrence, RecurrenceSchedule.ScheduleFieldSpec field, int minValidValue, int maxValidValue, List<ValidationFailureDetails> validationFailures) {
        boolean ret = true;
            ...
            if (StringUtils.isNotEmpty(spec)) {
                ...
                if (spec.startsWith("-") || spec.endsWith("-")) {
                    ...
                    if (ranges.length > 2) {
                        ...
                        if (!StringUtils.equals(ranges[0], RecurrenceSchedule.WILDCARD)) {
                            ...
                            value = RecurrenceSchedule.WILDCARD;
                        }
                        if (!StringUtils.equals(ranges[1], RecurrenceSchedule.WILDCARD)) {
                        ...
                        if (!StringUtils.equals(ranges[0], RecurrenceSchedule.WILDCARD)) {
                            ...
                            if (val < minValidValue || val > maxValidValue) {
                                ...
                                if (!StringUtils.equals(value, RecurrenceSchedule.WILDCARD)) {
    ...
    private String getNormalizedValue(RangerValidityRecurrence recurrence, RecurrenceSchedule.ScheduleFieldSpec field) {
        String ret = null;
