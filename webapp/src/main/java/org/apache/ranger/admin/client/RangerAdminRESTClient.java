...
import static org.apache.atlas.discovery.EntityDiscoveryService.directIndexSearchStatic;
...
public class RangerAdminRESTClient extends AbstractRangerAdminClient {
	...
	private ServicePolicies getServicePoliciesIfUpdatedWithCred(final long lastKnownVersion, final long lastActivationTimeInMillis) throws Exception {
		...
		if (response == null || response.getStatus() == HttpServletResponse.SC_NOT_MODIFIED || response.getStatus() == HttpServletResponse.SC_NO_CONTENT) {
			...
			List<AtlasEntityHeader> entityHeaders = getPoliciesFromAtlas();
			for(AtlasEntityHeader entityHeader: entityHeaders) {
				RangerPolicy rangerPolicy = new RangerPolicy();
				List<RangerPolicy.RangerPolicyItem> rangerPolicyItems = new ArrayList<>();
				List<AtlasObjectId> tempItems = (List<AtlasObjectId>) entityHeader.getAttribute("rangerPolicyItems");
				for (AtlasObjectId tempItem: tempItems) {
					Map<String,Object> tempAttributes = tempItem.getAttributes();
					RangerPolicy.RangerPolicyItem rangerPolicyItem = new RangerPolicy.RangerPolicyItem();
					rangerPolicyItem.setUsers((List<String>) tempAttributes.get("rangerPolicyItemUsers"));
					List<AtlasStruct> tempAccesses = (List<AtlasStruct>) tempAttributes.get("rangerPolicyItemAccesses");
					List<RangerPolicy.RangerPolicyItemAccess> rangerPolicyItemAccesses = new ArrayList<>();
					for (AtlasStruct tempAccess : tempAccesses) {
						RangerPolicy.RangerPolicyItemAccess rangerPolicyItemAccess = new RangerPolicy.RangerPolicyItemAccess();
						rangerPolicyItemAccess.setType((String) tempAccess.getAttribute("RangerPolicyItemAccessType"));
						rangerPolicyItemAccess.setIsAllowed((Boolean) tempAccess.getAttribute("RangerPolicyItemAccessIsAllowed"));
				...
				Map<String, String> tempResources =  (Map<String, String>) entityHeader.getAttribute("rangerPolicyResources");
				Map<String, RangerPolicy.RangerPolicyResource> resources = new HashMap<>();
				for (String key: tempResources.keySet()) {
					...
					List<String> values = new ArrayList<>(Arrays.asList(tempResources.get(key)));
					resource.setValues(values);
					resources.put(key, resource);
				}
				rangerPolicy.setResources(resources);
				rangerPolicy.setPolicyItems(rangerPolicyItems);
				rangerPolicy.setService((String) entityHeader.getAttribute("rangerPolicyService"));
				rangerPolicy.setName((String) entityHeader.getAttribute("rangerPolicyName"));
				rangerPolicy.setPolicyType((Integer) entityHeader.getAttribute("rangerPolicyType"));
				rangerPolicy.setPolicyPriority((Integer) entityHeader.getAttribute("rangerPolicyPriority"));
				rangerPolicy.setDescription((String) entityHeader.getAttribute("rangerPolicyService"));
				rangerPolicy.setIsAuditEnabled((Boolean) entityHeader.getAttribute("rangerPolicyIsAuditEnabled"));
				rangerPolicy.setId(1L);
				rangerPolicy.setVersion(1L);
				rangerPolicy.setGuid(entityHeader.getGuid());
				ret.getPolicies().add(rangerPolicy);
			}
	...
	public List<AtlasEntityHeader> getPoliciesFromAtlas() throws AtlasBaseException {
		IndexSearchParams indexSearchParams = new IndexSearchParams();
		Map<String, Object> dsl = getMap("from", 0);
		dsl.put("size", 100);
		dsl.put("query", getMap("term", getMap("__typeName.keyword", "RangerPolicy")));

		Set<String> attributes = new HashSet<>(Arrays.asList("rangerPolicyService", "rangerPolicyName", "rangerPolicyType", "rangerPolicyPriority", "rangerPolicyDescription", "rangerPolicyIsAuditEnabled", "rangerPolicyServiceType", "rangerPolicyPolicyLabels", "rangerPolicyIsDenyAllElse", "rangerPolicyZoneName", "rangerPolicyResources", "rangerPolicyItems"));
		Set<String> relationAttributes = new HashSet<>(Arrays.asList("rangerPolicyItemUsers", "rangerPolicyItemAccesses"));
		...
		searchResult = directIndexSearchStatic(indexSearchParams);
	...
	private ClientResponse getRangerAdminPolicyDownloadResponse(final long lastKnownVersion, final long lastActivationTimeInMillis, final UserGroupInformation user, final boolean isSecureMode) throws Exception {
		...
		Map<String, String> queryParams = new HashMap<String, String>();
		queryParams.put(RangerRESTUtils.REST_PARAM_LAST_KNOWN_POLICY_VERSION, Long.toString(0));
		queryParams.put(RangerRESTUtils.REST_PARAM_LAST_ACTIVATION_TIME, Long.toString(0));
		queryParams.put(RangerRESTUtils.REST_PARAM_PLUGIN_ID, pluginId);
